...
const UserOrders = () => {
  ...
  // We use a specific key based on UID so different users don't see cached data of others
  const getCachedOrders = () => {
    if (!currentUser) return [];
    const cached = localStorage.getItem(`nk_orders_${currentUser.uid}`);
  ...
  const [loading, setLoading] = useState(() => !localStorage.getItem(`nk_orders_${currentUser?.uid}`));
  
  const [expandedId, setExpandedId] = useState(null);

  // 2. BACKGROUND SYNC (The "Only Updated" logic)
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // Always fetch latest to ensure status (Shipped/Delivered) is accurate
        const q = query(collection(db, "orders_collection"), where("user_id", "==", userIdentifier));
        const querySnapshot = await getDocs(q);
        
        const freshData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        freshData.sort((a, b) => b.date - a.date); // Sort by Newest

        // 3. COMPARE AND UPDATE STORAGE
        // We compare strings to avoid unnecessary re-renders if data is identical
        const cachedStr = localStorage.getItem(`nk_orders_${currentUser.uid}`);
        const freshStr = JSON.stringify(freshData);

        if (cachedStr !== freshStr) {
           console.log("⚡ New data found, updating UI...");
           setOrders(freshData);
           localStorage.setItem(`nk_orders_${currentUser.uid}`, freshStr);
        } else {
           console.log("✅ Data up to date (Read from Cache)");
        }
      } catch (error) {
  ...
  const handleCancelOrder = async (order) => {
    ...
    try {
        ...
        const updatedList = orders.map(o => o.id === order.id ? { ...o, status: 'Cancelled' } : o);
        setOrders(updatedList);
        // Update Cache manually so next refresh knows it's cancelled
        localStorage.setItem(`nk_orders_${currentUser.uid}`, JSON.stringify(updatedList));
                  ...
                  <div style={{ borderTop: '1px solid #e2e8f0', padding: '20px', animation: 'slideDown 0.3s ease' }}>
                        ...
                        <div style={{ marginTop:'15px', paddingTop:'15px', borderTop:'1px solid #cbd5e1' }}>
                            ...
                            {order.discount_details?.code && (
                                <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.9rem', color:'#16a34a', marginBottom:'5px' }}>
                                    <span style={{ display:'flex', alignItems:'center', gap:'5px' }}><FaTag size={10} /> Coupon ({order.discount_details.code})</span>
                    ...
                    <div style={{ border:'1px solid #e2e8f0', borderRadius:'8px', padding:'15px', marginBottom: '20px' }}>
                       ...
                       <div style={{ display:'flex', flexDirection:'column', gap:'5px', fontSize:'0.9rem', color:'#4b5563', borderTop:'1px solid #f1f5f9', paddingTop:'10px' }}>
                          <div style={{display:'flex', alignItems:'center', gap:'8px'}}><FaPhone size={12} color="#94a3b8"/> {displayPhone}</div>
                          <div style={{display:'flex', alignItems:'center', gap:'8px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'100%'}}><FaEnvelope size={12} color="#94a3b8"/> {order.user_email || currentUser.email}</div>
                       </div>
                    </div>
                    ...
                    <div style={{ display:'flex', flexWrap:'wrap', gap:'15px' }}>
                        ...
                        {order.status === 'Pending' && (
                            <button onClick={() => handleCancelOrder(order)} className="btn" style={{ background:'#fee2e2', color:'#ef4444', border:'1px solid #fca5a5', padding:'12px 15px', fontSize:'0.9rem', flexGrow: 1, whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }}>
